...
int main() {
    ...
    try {
        while (true) {
            std::vector<uint8_t> response(64);  // Create a buffer for response
            ...
            if (bytesRead >= 8) {  // 2 bytes header + 3 * 2 bytes for ADC values = 8 bytes
                for (size_t i = 0; i <= bytesRead - 8; ++i) {
                    // Check for the header
                    if (response[i] == 0xAA && response[i + 1] == 0x55) {
                        // Decode the ADC values
                        uint16_t adcValues[3];
                        adcValues[0] = (response[i + 2] << 8) | response[i + 3];
                        adcValues[1] = (response[i + 4] << 8) | response[i + 5];
                        adcValues[2] = (response[i + 6] << 8) | response[i + 7];
