...
namespace CppLinuxSerial {
	...
	void SerialPort::SetBaudRate(BaudRate baudRate)	{
		std::cout << "standard Baudrate called\n";
		baudRateType_ = BaudRateType::STANDARD;
		baudRateStandard_ = baudRate;
        if(state_ == State::OPEN)
            ConfigureTermios();
	...
	void SerialPort::SetBaudRate(speed_t baudRate)	{
		std::cout << "custom Baudrate called\n";
		baudRateType_ = BaudRateType::CUSTOM;
		baudRateCustom_ = baudRate;
        if(state_ == State::OPEN)
            ConfigureTermios();
	...
	void SerialPort::Write(uint8_t* sendBuff, int buffLength){
        if(state_ != State::OPEN)
            THROW_EXCEPT(std::string() + __PRETTY_FUNCTION__ + " called but state != OPEN. Please call Open() first.");
		...
		for (int i = 0; i < buffLength; ++i) {
			std::cout << std::hex << std::uppercase << std::setw(2) << std::setfill('0') 
					<< static_cast<int>(sendBuff[i]) << " ";
		}*/

		int writeResult = write(fileDesc_, sendBuff, buffLength);
...
//		memset (&buf, '\0', sizeof buf);
	...
	int SerialPort::Read(uint8_t* rcvbuff, int buffLength){
		if(fileDesc_ == 0) {
			//this->sp->PrintError(SmartPrint::Ss() << "Read() was called but file descriptor (fileDesc) was 0, indicating file has not been opened.");
			//return false;
			std::cout << "fileDesc_ = " << fileDesc_ << std::endl;
			THROW_EXCEPT("Read() was called but file descriptor (fileDesc) was 0, indicating file has not been opened.");
		}

		//std::cout << "about to read from serial port" << std::endl;
		ssize_t n = read(fileDesc_, rcvbuff, buffLength); // read can never return greater than buffLength!
		...
		// Recursive read until we get the right number of bytes
		else if (n < buffLength){
			//if bytes read was less than expected, read again to fill in the remaining portion of the buffer
			ssize_t n2 = buffLength-n;
			Read(rcvbuff + n, n2);
		}
	...
	void SerialPort::FlushIO() {
		ioctl(fileDesc_, TCFLSH, 2); //clear both
	}
